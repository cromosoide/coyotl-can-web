"use client";

import { useState } from "react";
import { REVIEWS } from "@/lib/constants";

interface ResenasProps {
  limit?: number;
}

export default function Resenas({ limit }: ResenasProps) {
  const reviews = limit ? REVIEWS.slice(0, limit) : REVIEWS;
  const [current, setCurrent] = useState(0);

  return (
    <section className="bg-white py-16 sm:py-20">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="mb-12 text-center">
          <p className="mb-2 text-sm font-bold uppercase tracking-wider text-[#e5006b]">
            Reseñas
          </p>
          <h2 className="mb-4 text-2xl font-extrabold text-[#2d0057] sm:text-3xl">
            Lo que dicen nuestras familias
          </h2>
          {/* Google badge */}
          <div className="inline-flex items-center gap-2 rounded-full bg-[#f5f5f5] px-4 py-2">
            <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4" />
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
            </svg>
            <span className="text-sm font-bold text-[#2d0057]">5.0</span>
            <div className="flex text-[#fab200]">
              {[...Array(5)].map((_, i) => (
                <svg key={i} viewBox="0 0 20 20" fill="currentColor" className="h-4 w-4">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
              ))}
            </div>
            <span className="text-xs text-[#555]">8 reseñas</span>
          </div>
        </div>

        {/* Desktop: grid */}
        <div className="hidden gap-6 sm:grid sm:grid-cols-3">
          {reviews.map((review) => (
            <ReviewCard key={review.nombre} {...review} />
          ))}
        </div>

        {/* Mobile: carousel */}
        <div className="sm:hidden">
          <ReviewCard {...reviews[current]} />
          <div className="mt-4 flex justify-center gap-2">
            {reviews.map((_, i) => (
              <button
                key={i}
                onClick={() => setCurrent(i)}
                className={`h-2.5 w-2.5 rounded-full transition-colors ${
                  i === current ? "bg-[#e5006b]" : "bg-gray-300"
                }`}
                aria-label={`Ver reseña ${i + 1}`}
              />
            ))}
          </div>
        </div>
      </div>
    </section>
  );
}

function ReviewCard({
  nombre,
  mascota,
  texto,
  rating,
}: {
  nombre: string;
  mascota: string;
  texto: string;
  rating: number;
}) {
  return (
    <div className="rounded-2xl border border-gray-100 bg-[#f5f5f5] p-6">
      <div className="mb-3 flex text-[#fab200]">
        {[...Array(rating)].map((_, i) => (
          <svg key={i} viewBox="0 0 20 20" fill="currentColor" className="h-4 w-4">
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          </svg>
        ))}
      </div>
      <p className="mb-4 text-sm leading-relaxed text-[#555]">&ldquo;{texto}&rdquo;</p>
      <div>
        <p className="text-sm font-bold text-[#2d0057]">{nombre}</p>
        <p className="text-xs text-[#555]">{mascota}</p>
      </div>
    </div>
  );
}
