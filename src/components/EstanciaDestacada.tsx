...
export default function EstanciaDestacada() {
  ...
  return (
    <section className="relative overflow-hidden py-16 sm:py-20" style={{ backgroundColor: "#8002f2" }}>
      {/* Decorative pattern */}
      ...
      <div className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          <p className="mb-2 text-sm font-bold uppercase tracking-wider text-white/70">
            Estancia Coyotl
          </p>
          ...
          <div className="mb-10 flex flex-col items-center gap-4 sm:flex-row sm:justify-center sm:gap-8">
            {features.map((f) => (
              <div key={f.texto} className="flex items-center gap-3 text-white">
                <span className="text-2xl">{f.icono}</span>
                <span className="text-sm font-semibold">{f.texto}</span>
              </div>
            ))}
          </div>
          ...
          <a
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-white px-8 py-4 text-base font-bold text-[#8002f2] shadow-lg transition-all hover:bg-white/90 hover:shadow-xl active:scale-95"
          >
